		...
		if( $load_more ){
			...
			if( $paged < $total ){
				$output .= '<div class="column one pager_wrapper pager_lm">';
					$output .= '<a class="pager_load_more button button_js" href="'. next_posts(0,false) .'">';
...
function mfn_post_thumbnail( $postID, $type = false, $style = false, $load_more = false ){
	...
	if( is_single( $postID ) ){
		...
		if( ( $external = mfn_opts_get('portfolio-external') ) && $image_links ){
		...
		if( mfn_opts_get('portfolio-hover-title') ){
			...
			$link_after .= '<div class="image_links hover-title">';
				$link_after .= '<a href="'. get_permalink() .'">'. get_the_title() .'</a>';
			$link_after .= '</div>';
			...
			$link_after .= '<div class="image_links '. $image_links_class .'">';
				if( ! $external )$link_after .= '<a href="'. $large_image_url[0] .'" class="zoom" rel="prettyphoto"><i class="icon-search"></i></a>';
...
// 		),
	...
	$config = array(
		...
		'strings'      		=> array(
			...
			'complete' 									=> __( 'All plugins installed successfully. %s', 'betheme' ), // %1$s = dashboard link
